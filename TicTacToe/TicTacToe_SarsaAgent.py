...
class TicTacToeGame(object):
	def __init__(self, learning_rate=0.5, epsilon=0.8, episodes=500, write_statistics=True):
		...
		self.player1 = SarsaAgent('X', alpha=self.learning_rate, gamma=0.9, epsilon=self.epsilon)
...
class Statistics(object):
	...
	def write_statistic_file(self, player, eps, clock_time, cpu_time):
		filename = "trainingresults_sarsa_{}.txt".format(eps)
	...
	def write_qtable(player, eps):
		filename = "sarsa-table{}.json".format(eps)
		file = open(filename, "w+")
